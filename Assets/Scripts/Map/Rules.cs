...
public class MapRules
{
    public static bool MustPlaceBlock(int[] lastRow, int[] currentRow, int index)
    {
        ...
        if (MapUtils.GetRowStatus(currentRow, index, -1) == 1 &&
            ...
            MapUtils.GetRowStatus(lastRow, index) == 1)
        {
            return true;
        }
        ...
        if (index >= currentRow.Length - 3 &&
            MapUtils.GetRowStatus(currentRow, index, -1) == 1 &&
            MapUtils.GetRowStatus(lastRow, index) != -1 &&
            MapUtils.GetRowStatus(lastRow, index,1) != -1)
        {
            return true;
        }
    ...
    public static bool CanPlaceBlock(int[] lastRow, int[] currentRow, int index)
    {
        ...
        if (index == 1 && MapUtils.GetRowStatus(currentRow, 0) == -1 && MapUtils.GetRowStatus(lastRow, 0) == 1)
        {
            return false;
        }

        //Rule: Don't block if there is block left side and left blocks don't have a way up (make a U turn)
        if ((MapUtils.GetRowStatus(currentRow, index, -3) == 1 ||
    ...
    public static bool CanPlaceVerticalPortal(int[] portalRow, int index)
    {
        ...
        if (MapUtils.GetRowStatus(portalRow, index, -1) == -2 ||
            MapUtils.GetRowStatus(portalRow, index, 1) == -2)
        {
            return false;
        }
        return true;
